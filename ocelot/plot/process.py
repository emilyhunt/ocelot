...
def kde_fit_2d(xy: np.ndarray, bandwidth: float,
               x_limits: Optional[List[float]] = None, y_limits: Optional[List[float]] = None,
               resolution: int = 50):
    ...
    Notes:
        ...
        resolution of scoringwill result in faster running.

    Args:
        xy (np.ndarray): the x and y data, of shape (n_samples, n_features).
        bandwidth: the bandwidth to be used by the KDE. Recommended: about 0.08.
        ...
        resolution (int): number of points to sample the KDE at when scoring
    ...
    grid_x = np.linspace(x_limits[0], x_limits[1], resolution)
    grid_y = np.linspace(y_limits[0], y_limits[1], resolution)
    mesh_x, mesh_y = np.meshgrid(grid_x, grid_y)

    # Fit the KDE
    xy_kde = KernelDensity(bandwidth=bandwidth,
                           metric='euclidean',
                           kernel='gaussian',
                           algorithm='ball_tree')
    xy_kde.fit(xy)
    ...
    z = xy_kde.score_samples(np.array(
        [mesh_x.flatten(), mesh_y.flatten()]).T)
