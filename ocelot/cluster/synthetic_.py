...
from ..calculate.random import random_spherical
...
class SimulatedPopulations:
    def __init__(self,
        ...
        self.unique_log_ages = np.round(np.unique(self.data["log_age"].to_numpy()), decimals=2)
    ...
    def _add_positions_to_cluster(new_population: pd.DataFrame, data_cluster: pd.Series):
        ...
        # Draw random r values and a number between 0 and max_king to decide whether or not to accept the r value.
        ...
        theta, phi = random_spherical(total_stars, radians=False)
    ...
    def _add_proper_motions_to_cluster(new_population: pd.DataFrame, data_cluster: pd.Series):
        ...
        cluster_stars = SkyCoord(cluster_stars.with_differential(cluster_star_velocities), frame='icrs')
    ...
    def _add_photometry_to_cluster(new_population: pd.DataFrame, data_cluster: pd.Series):
        ...
        new_population['phot_g_mean_mag'] += (
                data_cluster['distance_modulus'] + data_cluster['extinction'] * gaia_dr2_a_lambda_over_a_v["G"])
        new_population['phot_bp_mean_mag'] += (
                data_cluster['distance_modulus'] + data_cluster['extinction'] * gaia_dr2_a_lambda_over_a_v["G_BP"])
        new_population['phot_rp_mean_mag'] += (
                data_cluster['distance_modulus'] + data_cluster['extinction'] * gaia_dr2_a_lambda_over_a_v["G_RP"])
    ...
    def _draw_cluster_of_correct_mass(self, current_population: pd.DataFrame, target_mass: float,):
        ...
        while np.abs(mass_difference) > target_mass * self.mass_tolerance:
            ...
            if mass_difference > 0:
                np.append(new_indices, np.random.randint(0, high=n_simulated_stars, size=n_stars_to_draw))
            else:
                np.delete(new_indices, np.random.randint(0, high=new_indices.shape[0], size=n_stars_to_draw))
    ...
    def get_clusters(self, data_clusters: pd.DataFrame,
        ...
        data_clusters['age'] = self.unique_log_ages[np.argmin(age_differences, axis=1)]
        data_clusters['Z'] = self.unique_log_metallicities[np.argmin(metallicity_differences, axis=1)]

        # Check the user isn't being a numpty
        if error_on_invalid_request:
            if np.any(np.logical_or(data_clusters['age'] > 1, data_clusters['Z'] > 1)):
