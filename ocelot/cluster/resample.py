...
def generate_gaia_covariance_matrix(data_gaia: pd.DataFrame, six_parameter_sources: bool = False) -> np.ndarray:
    ...
    if six_parameter_sources:
        ...
        return np.asarray(
            [[pmra_pmra,     pmra_pmdec,     pmra_parallax,     pmra_color],
             ...
             [pmra_color,    pmdec_color,    parallax_color,    color_color]]).T
...
def resample_gaia_astrometry(data_gaia: pd.DataFrame,
                             check_valid: str = 'ignore', method: str = "svd", n_resamples: int = 1,
                             suffixes: Optional[Union[list, tuple, np.ndarray]] = None) -> pd.DataFrame:
    ...
    if np.any(five_parameter_astrometry):
        ...
        resampled_astrometry[five_parameter_astrometry] = vectorized_multivariate_normal(
    ...
    if np.any(six_parameter_astrometry):
        covariance_matrix_six = generate_gaia_covariance_matrix(data_gaia_six, six_parameter_sources=True)

        resampled_astrometry[six_parameter_astrometry] = vectorized_multivariate_normal(
            mean=data_gaia_six[['pmra', 'pmdec', 'parallax', 'pseudocolour']].to_numpy(),
    ...
    for i in range(n_resamples):
        ...
        resampled_astrometry_dict[f"pmdec_{suffixes[i]}"] = resampled_astrometry[:, i, 1]
        resampled_astrometry_dict[f"parallax_{suffixes[i]}"] = resampled_astrometry[:, i, 2]
