...
def _calculate_m_over_h(
    z,
    z_over_x_solar: float = 0.0207,
    y_constant: float = 0.2485,
    z_multiplier: float = 1.78,
):
...
class SimulatedPopulations:
    def __init__(
        ...
        position_oversampling_factor: Union[float, int] = 1.0,
        limiting_magnitude: float = 22.0,
        ...
        self.required_info_on_clusters = [
            "ra",
            "dec",
            "distance",
            "extinction_v",
        ...
        self.data = pd.concat(separate_dataframes, ignore_index=True).reset_index(
            drop=True
        )
    ...
    def _add_positions_to_cluster(
        self, new_population: pd.DataFrame, data_cluster: pd.Series
    ):
        ...
        max_king = king_surface_density(
            0, data_cluster["radius_c"], data_cluster["radius_t"]
        )
        ...
        inverse_normalisation_constant = max_king / king_surface_density(
            0, data_cluster["radius_c"], data_cluster["radius_t"], normalise=True
        )
        ...
        expected_acceptance_fraction = (
            inverse_normalisation_constant / (data_cluster["radius_t"] * max_king)
        ) ** 3
        ...
        while remaining_stars > 0:
            ...
            stars_to_draw = int(
                remaining_stars
                * self.position_oversampling_factor
                / expected_acceptance_fraction
            )
            test_r_values = (np.random.rand(stars_to_draw, 3) * 2 - 1) * data_cluster[
                "radius_t"
            ]
            ...
            # Evaluate the King profile at these points
            king_values = king_surface_density_fast(
                np.abs(test_r_values),
                data_cluster["radius_c"],
            ...
            if n_good_test_values > remaining_stars:
                ...
                index_when_we_have_enough_values = (
                    (np.cumsum(good_test_values) == values_to_write).nonzero()[0]
                )[0]
            ...
            r_values[
                completed_stars : completed_stars + values_to_write
        ...
        cluster_location = SkyCoord(
            ra=data_cluster["ra"] << u.deg,
            dec=data_cluster["dec"] << u.deg,
            distance=data_cluster["distance"] << u.pc,
        ).cartesian

        cluster_stars = CartesianRepresentation(r_values << u.pc, xyz_axis=1)
    ...
    @staticmethod
    def _add_proper_motions_to_cluster(
        new_population: pd.DataFrame, data_cluster: pd.Series
    ):
        ...
        v_x, v_y, v_z = norm.rvs(
            loc=0.0, scale=data_cluster["v_int"], size=(3, new_population.shape[0])
        )

        # Add these velocities to the cluster stars' locations to get proper motions
        cluster_stars = SkyCoord(
            ra=new_population["ra"] << u.deg,
            dec=new_population["dec"] << u.deg,
            distance=new_population["distance"] << u.pc,
            frame="icrs",
        ).cartesian
        ...
        cluster_stars = SkyCoord(
            cluster_stars.with_differentials(cluster_star_velocities), frame="icrs"
        )
    ...
    def _add_photometry_to_cluster(
        self, new_population: pd.DataFrame, data_cluster: pd.Series
    ):
        ...
        binary_offsets[:n_binaries] = np.sign(
            self.binary_offset
        ) * np.random.exponential(np.abs(self.binary_offset), size=n_binaries)
        binary_offsets = np.random.permutation(binary_offsets)

        # Make the magnitudes
        new_population["phot_g_mean_mag"] += (
            data_cluster["distance_modulus"]
            + data_cluster["extinction_v"] * gaia_dr2_a_lambda_over_a_v["G"]
            + binary_offsets
        )
        new_population["phot_bp_mean_mag"] += (
            data_cluster["distance_modulus"]
            + data_cluster["extinction_v"] * gaia_dr2_a_lambda_over_a_v["G_BP"]
        )
        new_population["phot_rp_mean_mag"] += (
            data_cluster["distance_modulus"]
            + data_cluster["extinction_v"] * gaia_dr2_a_lambda_over_a_v["G_RP"]
        )
        new_population["bp_rp"] = (
            new_population["phot_bp_mean_mag"] - new_population["phot_rp_mean_mag"]
        )
    ...
    def _draw_cluster_of_correct_mass(
        self, simulated_population: pd.DataFrame, target_mass: float
    ):
        ...
        new_indices = np.random.randint(0, high=n_simulated_stars, size=stars_to_draw)
        ...
        while np.abs(mass_difference) > target_mass * self.mass_tolerance:
            ...
            if mass_difference > 0:
                new_indices = np.append(
                    new_indices,
                    np.random.randint(0, high=n_simulated_stars, size=n_stars_to_draw),
                )
            else:
                new_indices = np.delete(
                    new_indices,
                    np.random.randint(
                        0, high=new_indices.shape[0], size=n_stars_to_draw
                    ),
                )
            ...
            if iterations > self.max_mass_iterations:
                raise ValueError(
                    f"unable to select a cluster of the correct mass after {iterations} iterations! Try "
    ...
    def _make_cluster(self, data_cluster: pd.Series, cluster_label: int):
        ...
        good_stars = np.logical_and(
            self.data["log_age"] == data_cluster["age"],
        ...
        if np.count_nonzero(good_stars) == 0:
            raise ValueError(
                f"specified age {data_cluster['age']} and metallicity {data_cluster['Z']} combination "
                f"was not found in the simulated population!"
            )

        # Turn this into a new DataFrame! (I don't want to mess up the old one)
        simulated_population = (
            self.data.loc[good_stars, :].copy().reset_index(drop=True)
        )

        # Shuffle the DataFrame and create a stellar sample of roughly the required mass
        new_population = self._draw_cluster_of_correct_mass(
            simulated_population, data_cluster["mass"]
        )
        ...
        new_population = new_population.loc[
            new_population["phot_g_mean_mag"] < self.limiting_magnitude, :
        ].reset_index(drop=True)
        new_population = self._add_positions_to_cluster(new_population, data_cluster)
        new_population = self._add_proper_motions_to_cluster(
            new_population, data_cluster
        )
    ...
    def get_clusters(
        ...
        age_differences = np.abs(
            np.asarray(data_clusters["age"]).reshape(-1, 1) - self.unique_log_ages
        )
        metallicity_differences = np.abs(
            np.asarray(data_clusters["Z"]).reshape(-1, 1)
            - self.unique_log_metallicities
        )
        ...
        if error_on_invalid_request:
            if np.any(
                np.logical_or(
                    age_differences[np.arange(n_stars), indices_best_age] > 1,
                    metallicity_differences[
                        np.arange(n_stars), indices_best_metallicity
                    ]
                    > 1,
                )
            ):
                raise ValueError(
                    "an input age or metallicity value is too different (more than 1) from the values \n"
        ...
        while i_cluster < n_clusters:
            clusters_to_return.append(
                self._make_cluster(data_clusters.loc[i_cluster, :], i_cluster)
            )
            i_cluster += 1

        # Concatenate if requested and return
        if concatenate:
            return pd.concat(clusters_to_return, ignore_index=True).reset_index(
                drop=True
            )
        else:
...
def _c_median_plus_or_minus_1_mad(data, size, name):
    return (
        np.median(data[name])
        * np.asarray([+1, -1])
        * median_absolute_deviation(data[name])
    )
...
def _setup_cluster_parameter(
    ...
    # Otherwise, if it's a function, then we want
        ...
        raise ValueError(
            "specified desired_parameter_values has an unsupported type. "
...
magnitude_parameters = ["phot_g_mean_mag", "phot_bp_mean_mag", "phot_rp_mean_mag"]

magnitude_parameter_band_names = ["G", "G_BP", "G_RP"]

bp_rp_color_name = "bp_rp"

flux_parameters = ["phot_g_mean_flux", "phot_bp_mean_flux", "phot_rp_mean_flux"]

parameters_to_assign = ["ruwe"]

distance_parameters_with_assymetric_error = ["distance"]
...
distance_parameters_with_assymetric_error_keys = {
    "distance": ["r_est", "r_lo", "r_hi", "parallax"]
}


def _find_nearest_magnitude_star(
    gaia_photometry: Union[pd.Series, np.ndarray],
    ...
    simulated_photometry += np.random.normal(
        loc=0.0, scale=fiddle_factor, size=simulated_photometry.shape
    )
    ...
    return np.argmin(
        np.abs(
            np.asarray(gaia_photometry).reshape(1, -1)
...
def _convolve_simulated_clusters_with_errors(
    data_gaia: pd.DataFrame, data_simulated: pd.DataFrame
):
    ...
    if data_gaia.shape[0] != n_stars:
        raise ValueError(
            "data_gaia must already have been pre-selected to be the best matches to data_simulated, and "
    ...
    for a_parameter in distance_parameters_with_assymetric_error:
        ...
        data_simulated[a_parameter_mode] = data_simulated["distance"] * (
            1
            + np.random.randn(n_stars)
            * gaia_fractional_errors[np.arange(n_stars), use_low_or_high]
        )

        data_simulated[a_parameter_low] = data_simulated[
            a_parameter_mode
        ].to_numpy() * (1 + gaia_fractional_errors[:, 0])
        data_simulated[a_parameter_high] = data_simulated[
            a_parameter_mode
        ].to_numpy() * (1 + gaia_fractional_errors[:, 1])
    ...
    for a_parameter in parameters_with_symmetric_error:
        ...
        data_simulated[a_parameter] += np.random.normal(
            0.0, scale=data_simulated[a_parameter_error], size=n_stars
        )
    ...
    for a_magnitude_parameter, a_band, a_flux in zip(
        ...
        data_simulated[a_magnitude_parameter] = (
            -2.5 * np.log10(data_simulated[a_flux]) + gaia_dr2_zero_points[a_band]
        )

    # Add a new bp-rp colour
    if bp_rp_color_name is not None:
        data_simulated[bp_rp_color_name] = (
            data_simulated["phot_bp_mean_mag"] - data_simulated["phot_rp_mean_mag"]
        )
...
def generate_synthetic_clusters(
    simulated_populations: SimulatedPopulations,
    data_gaia: pd.DataFrame,
    ...
    if mode not in _valid_modes:
        raise ValueError(
            f"specified mode {mode} not supported! You may choose from the following values: \n"
            f"{_valid_modes}"
        )
    ...
    for a_key in cluster_parameters.keys():
        data_cluster[a_key] = _setup_cluster_parameter(
            data_gaia, a_key, cluster_parameters[a_key], n_clusters
        )

    # Cluster simulation time!!!!
    data_simulated = simulated_populations.get_clusters(
        data_cluster, **kwargs_for_simulated_populations
    )

    # Convolve some cheeky errors in there too
    star_matches = _find_nearest_magnitude_star(
        data_gaia["phot_g_mean_mag"], data_simulated["phot_g_mean_mag"]
    )
